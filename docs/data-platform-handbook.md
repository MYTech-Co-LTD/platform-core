...
> 按**硬约束**逐条对账。本表的 **C1 / C2 / C3** 是公司标准（`team-harness`，**尚未合入**）§2 的编号——本正典对这几条的表述见 §1.1.2（落盘即定型 / 字段集稳定 / 身份列）。**这些是「已落盘但不符合约束」的存量**，重构时一并改。⚠️ 本表的欠账描述的是**旧前缀** `lemeng/retail_detail/…` 的存量；新湖口径见 §1.7。
