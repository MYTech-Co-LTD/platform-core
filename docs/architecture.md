...
- **职责**：**唯一对外门 + 治理面**——词表裁剪与 org 谓词（`domain/authz.ts`）、
  ...
  **不构成「不在词表即不可见」**；划线口径见本节末注（2026-09-26 订正）。
...
> **AI 洞察：入口唯一，授权划线，语义层管口径**（2026-09-26 订正，取代本注原先的「为什么 AI 通路
> 只有一条」）。**入口不变**——仍是上面第 6 条的 `modules/data`，**AI 不直连数据面**（这条判断还被
> §2.3 的「售后直连数据面」拒斥理由引用，一并保持有效）。**改的是「门内怎么划线」**，以及
> **三种角色各管哪一段、不互相兼任**：
>
> | 角色 | 管什么 | **不管**什么 |
> |---|---|---|
> | **统一身份认证 + 权限认证**（Casdoor 身份 + `modules/data/domain/authz.ts` 的 org 谓词） | **能看到哪些数据**（租户 / 主体 / 行级） | 不管口径怎么定 |
> | **dbt 语义层**（`dbt/semantics/l1_metrics.yml` + marts `schema.yml`） | **指标怎么定义**——供 AI **理解**、保**全局口径一致** | **不是查询闸**，**不限定能查什么** |
> | **AI 洞察** | 在**授权范围内任意探索**（含**明细层**，经 pg_duckdb 读，吃 DuckDB 的性能） | 不越授权、不绕审计 |
>
...
> ⇒ **两条不放松**：**探索纳入审计**（落 `data.query_audit`，与问数同一套 `domain/audit-store.ts`）；
> **超时 / 行数上限是工程保护**（防一次查询拖垮数据面），**不当权限讲**。
>
> ⚠️ **本注是目标形态，不是现状——能力尚未开通（文档先行）**。它成立的前提是「**授权面本身可靠**」，
> 而该前提**还没做实**，两条缺口都开着：
> **缺口 A** —— `marts` 关系**缺 org 列**（issue **#176**）⇒ **行级授权没有着力点**，AI 一读即跨租户；
> **缺口 B** —— 租户隔离的**承重点未验**（**#174**：`scope` 作 USER MAPPING OPTION 无上游依据、
> reconcile 对凭据面全盲；**#175**：每租户一桶在实现里降级为共享桶 + 前缀，待裁决）。
> ⇒ **缺口 A / B 落地之前，不得按本注开通任何「自由探索」通路**。顺序是「**先补授权面 →
...
| 反模式 | 为什么不做 |
|---|---|
| **售后直连数据面 pg_duckdb** | OLTP 打 OLAP（连接模型与资源模型都错位）；**跨部署单元**（单元 A↔单元 B，B 全内网不对平台进程开放接线）；**权限模型不合**（数据面不持有平台身份，`modules/data` 才是唯一门——§2.2 末注） |
...
| 同上 §11.3 | AI 走 **duckdb-ossie MCP** 问数据（两条 AI 通路） | **已废止**（2026-09-26 订正）——废止的是 `duckdb-ossie` **那套实现**；**入口仍唯一**（AI 不直连数据面），改的是**门内划线**：由**授权面**划、**语义层只管道径**。理由与两条前置缺口见 §2.2 末注 |
| 同上 §11.3.1 | 「**保护来自每租户会话绑定**」⇒ 任何绕开 facade 的数据面直连（含 **Metabase 官方 MCP**）不合规 | **（2026-09-26）两半分开**：「**入口唯一、不直连数据面**」**保留**；但**保护的落点从「会话绑定」改为「授权面」**（租户 / 主体 / 行级），`filter allowlist` 一类**查询形状约束不算权限**（§11.3.1 那条观察仍成立）。⚠️ 「**Metabase 官方 MCP**」一项的**判据随之改写**（从「是否经 facade」→「是否受授权面约束 + 是否入审计」）——**本轮未重新裁决** |
