...
- **凭证放哪 —— 分两阶段（2026-09-16 裁定「方向 C」）**：
  ...
  - **目标形态（M3c 收口）**：**每租户可配**——凭证落**租户行**，与 §1.3 公众号
    `wechat_oa_app_id/secret` **同构**；env 退化为**平台默认**（租户未配时用平台桶）。
    **「启用判定」判的是「是否 BYO」（2026-09-16 裁定 (a)）**：租户行有配置 = 该租户自带存储，
    无 = 用平台桶；**「启用」是运营/UI 概念，不影响 503 语义**——配了但连不上，照样由模块回 503。
    （本节旧措辞「启用判定 = 租户行存储配置存在」与同段的「env 作兜底」并存时**自相矛盾**：无配置
    的租户经由平台桶照样有可用存储，那句判的到底是什么？已按裁定 (a) 写准。）
  ...
  - **M3c 的前置（架构先行）**：`ModuleContext` 目前**只有 `pool`**
    ...
    `docs/module-protocol.md`，再动码**。（**2026-09-16：文档已落**——`architecture.md` §4.3 +
    §5 第 9 条、`module-protocol.md`「租户级配置注入」；**码仍未动**，见 §7 修订记录。）
...
- ⚠️ **附件行需加 `storage_ref`**（2026-09-16 裁定①：**纳入 M3c 范围内，不另开一期**）：
  `ticket_attachment` 现在**只有 `object_key` 一列，没有 bucket / endpoint**，而预签名用的是
  **当次请求解析出的配置** ⇒ 租户的存储配置一旦变化（未配 → 配了自己的桶 / 换桶 / 换 AK 对应账号 /
  回滚清空配置），**存量行的下载链接会指向新桶** ⇒ `NoSuchKey`；客户端表现为「**附件打不开**」，
  而平台侧**静默**——预签名是纯本地计算、不发网络请求，平台发不出这个错
  （见 `docs/module-protocol.md`「租户级配置注入」的「兜底语义」）。
  ⇒ **写入附件行时记下写入当时的配置标识**（最小可用：`bucket` + `endpoint`），读时按它取配置。
  三个候选里这是**唯一不依赖人记性**的一条：「配置冻结」无法技术强制（平台不联网就不知道桶里有没有
  对象），「接受断链 + 人工迁移」与 `~/.claude/rules/common/deploy-verify.md` §3「别用容错掩盖失败」
  的精神相反。
  **代价如实记**：改 aftersales 的迁移 + 写入路径 ⇒ **跨出「纯协议」范围**，仍是 M3c 的一部分。
  ⚠️ **边界（`storage_ref` 解决到哪一步为止）**：它解决的是「**指向哪个桶**」（`bucket` + `endpoint`），
  **不解决「用哪套凭据读它」**——凭据是密钥、不进行。所以上面第三种触发场景（回滚清空租户行）里，
  桶是认得出的、凭据却没了，读旧对象仍可能失败；「换 AK 且换了账号主体」同理。
  这一类**只能靠保存时的连通性探测提示 + 人工处置**（探测见 `docs/module-protocol.md`「租户级配置注入」
  的「兜底语义」）。
  要不要把「读旧对象所需的凭据」也纳入 `storage_ref` 的表示（如记配置标识而非值，再靠配置历史回取）
  是**落地前待定的一个小决定**，写在实现计划里，本文不预设。
...
| **M3c 每租户可配 ZOS** | 凭证落租户行 + `platform.tenant` 加列 + 配置 UI + **模块接入协议扩展**（§2.3）+ **附件行加 `storage_ref`**（§2.3，2026-09-16 裁定①，含模块侧写入路径）；**协议文档先行**（`architecture.md` + `module-protocol.md`，已落）。**不在试点关键路径上**（单租户形态下 env 成立），故排在 M3a/M3b-1/M3b-2 之后 |
...
- 2026-09-16（**M3c 拍板结论落进架构与协议文档**——提案「架构先行」的第二步。**本轮只改文档，
  实现代码一行未动**）：
  提案（`docs/superpowers/specs/2026-09-16-m3c-tenant-storage-protocol-design.md`）经人拍板 7 条
  （清单见其 §8），本轮把结论写进**规则正文**：`docs/architecture.md` 新增 **§4.3**（机制 + 逐条
  不变量对照）与 **§5 扩展点第 9 条**；`docs/module-protocol.md` 新增整节**「租户级配置注入：
  `storage`」**（manifest 可选字段形状与语义、context 键 `TENANT_STORAGE` 与 `TenantStorageConfig`、
  注入只在**模块 API 子树**、**不声明就不 set**、「声明的是能力不是租户」的安全性质、兜底语义、
  「加而不改」的兼容性论证）。
  **本文件 §2.3 两处订正**：
  ① **「启用判定」按裁定 (a) 写准**——它判的是**「是否 BYO」**（租户行有配置 = 自带存储），
  是**运营/UI 概念、不影响 503 语义**；原文两句自相矛盾（既说 env 作兜底，又说「启用 = 租户行
  存储配置存在」，而走平台桶的租户照样有可用存储）；
  ② **记一笔：附件行需加 `storage_ref`**（裁定①，**M3c 范围内**）——只记 `object_key` 而预签名
  用当次解析的配置 ⇒ 配置一变，存量行下载链接指向新桶 ⇒ `NoSuchKey`，客户端表现为「附件打不开」
  而**平台侧静默**（预签名是本地计算，平台发不出这个错）。并记下它的**边界**：`storage_ref` 解决
  「指向哪个桶」，**不解决「用哪套凭据读它」**（凭据是密钥、不进行）——回滚清空租户行这类场景里
  桶认得出、凭据没了，仍需人工处置。
  ⚠️ **只动了文档**：SDK 类型 / manifest schema / loader 投影中间件 / 模块侧的改造**均未开始**，
  待后续期；`docs/module-protocol.md` 那节是**待落地**的契约（照本仓规矩，文档先行）。
