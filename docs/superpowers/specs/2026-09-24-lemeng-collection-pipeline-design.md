...
| # | 域 | ability | 账套 | 节奏（§6） | 自然键（行粒度） | 量级（实测） |
|---|---|---|---|---|---|---|
| 1 | 零售明细 | `nhsoft.retail.ai.pos.posorder.find` | **双** | 5min 时段窗 + 多层重放 | `(order_no, order_detail_num)` | 峰值 **1459 单/时**（2026-09-24 19 点，生产实测，见 §6/#197） |
| 2 | 调拨单（内部配送源） | `nhsoft.ama.ai.transfer.out.order.find` | 3120 | 5min 增量 + 每小时全量 + 回溯 | `(order_no, item_num, lot_number)`（2026-09-24 实测：行无 id/序号列；单据内同品按批次拆行致 item_num 重复（100 单 655 行中 8 单重复），`(item_num, lot_number)` 残余重复 0；lot_number 有 121/655 行为空串/NULL，入键需 COALESCE） | 本任务未测 |
...
1. **分页双风格**：#1/#5/#6/#7 用 body `page_number/page_size`（**文档实写上限 200**；本仓发布版 `page_size=200 × 12 页`，容量口径与实测见 §6。早期记的「上限 100」是误读——100 在峰值窗需 14 页、末页哨兵必命中，而该窗真机全量跑通过 ⇒ 网关确实认 200）；#3/#4 用 `paging/limit/offset`；query 参数一律无效。
...
失败处理：job 失败 = openship job 告警（失败可见，不静默）；下一 tick 自然重试窗口（重叠式窗口天然容错一次漏拉）。**日粒度 job（S1 现行）另有一条**：它只采「昨天」且**无回溯重放**（回溯属 S3）⇒ 「一窗失败即停」等于**失败点之后的窗口当天永久缺失**；故 `windows` 取**逐窗尽力采**——单窗失败仍续采，失败窗逐一登记、**末尾**统一判红（连续 `WINDOWS_MAX_CONSEC` 窗失败则中止，避免空转撞 job 超时）；配 job 级 `retry` 给整轮第二次机会。见 #209。观测：`_ops` 指标（行数/页数/窗口/耗时）由 job wrapper 写 OpenObserve（文件日志按 SOP 接入）——⚠️ **该通道在数据面机上尚未接通**（`/etc/openobserve-ingest.env` 缺失，wrapper 每窗打 `OPS_SINK=DISABLED`），见 #210。
...
**零售链路的页容量口径（2026-09-25 扩容版；与 §3.1 #1 的「上限 200」同源）**：`page_size=200 × 12 页`，其中 **p12 是哨兵页**（挂 `ctl.die`：p12 仍有行 ⇒ 容量截断，中止——见实施计划的 Task 7）⇒ **真实静默通过阈值 = 11×200 = 2200 单/时**；2201–2400 区间落在 p12 上 ⇒ 是 **fail-loud 不丢数**（不是静默丢）——**余量一律按 2200 算，不要写 2400**。扩容前是 `200 × 8 页`（阈值 1400）；因**生产实测**峰值窗 **2026-09-24 19 点 1459 单**击穿 1400（哨兵命中、窗口断在 19 点），按 issue **#197** 扩容到 12 页。当前口径：**利用率 1459/2200 = 66.3%**、**余量 741 单（占阈值 33.7%）**；对照扩容前——阈值 **2200/1400 = 1.57×**、余量 **741/89 = 8.3×**。⚠️ **2200 只是实测峰值的 1.51×**（要到 2×1459 = 2918 需 ≥15 页 = 2800 仍不足、16 页 = 3000 才过 2×）⇒ #197 正文「余量回到 2 倍以上」若按「≥2× 峰值」读**未达成**、按「余量 ≥ 旧余量 2 倍」读已达成；**两种读法都留在此处，不合并、不宣称「2 倍达成」**。

**已知残余风险（12 个页请求不是一个快照）**：12 个页请求**不是同一快照**——请求之间到达的单会让分页边界漂移，理论上一行可能**重复或被跳过**（页数 8 → 12 使漂移面相应变大：每窗多 4 个分页边界，自愈机制不变）。当前的自愈是「整分区**覆盖写** + staging 的**自然键唯一性断言**」（重复 ⇒ 断言红，见 S1 的 `assert_stg_lemeng_retail_order_line_key_unique`）；S3（在线全域 5min 调度）会放大这个面——同一窗口 5 分钟一重拉 ⇒ 漂移机会更多。
