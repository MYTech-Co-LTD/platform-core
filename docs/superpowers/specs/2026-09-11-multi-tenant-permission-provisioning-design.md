...
即复评员手工探针的形状 —— 断言全链路**仍通过**。

修复前这条断言**因错误原因通过**（读侧根本没用它）；修复后必须因**正确原因**通过。
beta-403 断言正是「原因确实不同」的证明：若供给仍依赖 `PLATFORM_ORG`，
beta 的码就不会存在，而 beta-403 会与「码不存在」也 403 的情况**无法区分** ——
故 beta 侧必须同时断言「码**存在**」（mock 可查证）而「授权**不存在**」。
