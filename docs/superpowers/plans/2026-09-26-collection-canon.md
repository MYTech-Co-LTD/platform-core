...
| **D 管线** | duckle 管线，**由真引擎产出**（MCP `create_pipeline` / `validate_pipeline`），**仓内不手编**；含 `data.schema` + `qa.contract` gate + `drift` 门禁 + 末页守卫 | 引擎 `validate` 过；`drift` 有效（**先断言声明存在**，防假绿，§1.3.2 坑 1） | 回退到上一版管线 JSON |
