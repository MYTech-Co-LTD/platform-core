...
> （漏的是第 5 项「跨组件 parquet 格式兼容」——该项是本仓切官方镜像时才新引入的面。⚠️ 初稿在这里写成「漏的是非回环 `UNCLAIMED` 分支」是**错的**：`UNCLAIMED` 本就是那四项里的第 4 项，见 `deploy/duckle/README.md:249-250` 与 `docs/superpowers/plans/2026-09-22-data-stack.md:1210-1211` 的旧四项枚举）。**照着抄就会把这个错一起抄进正典**。
```
...
> 有牙的判据是这三条**同时**成立：`grep -c "细节见 §7 待沉淀 #3"` = `0`（**初版**那格确已消失）**且** `grep -c "细节见 §1.6 案例库"` = `0`（**过渡版**那格也已撤）**且** `grep -c "细节待核实后补"` ≥ `1`（终版已落地）。
>
> ⚠️ **两条踩过的坑，别再犯**：① 初稿的「原文」是**转述**而非逐字串，导致守卫改前即为 0（**永不可能红**）；
> ② 给出的守卫图案本身**不可能匹配**逐字文本（如要求 `**UTC**` 紧贴 `* * *`，而正文里两者之间有一个收尾反引号）⇒ 实现者只能在「违反逐字」与「违反守卫」间二选一。
> **写守卫前先拿它跑一遍改前/改后的真实文本**——不能红的守卫与不能绿的守卫都是假绿。
...
**Expected:** 两行 OK；`细节待核实后补` 计数 ≥ 1；`细节见 §7 待沉淀 #3` 与 `细节见 §1.6 案例库` 计数**均为 `0`**；登记表行数 ≥ 6；占位计数 `0`。
...
| 位置 | 原文 | 改为 |
|---|---|---|
| §4 **标题** | `## 4 已知欠账（对照标准的差距）` | `## 4 已知欠账（对照硬约束的差距）` |
| §4 开头引用块 | `> 按标准 §2 硬约束逐条对账；**这些是「已落盘但不符合标准」的存量**，重构时一并改。` | `> 按**硬约束**逐条对账。本表的 **C1 / C2 / C3** 是公司标准（`team-harness`，**尚未合入**）§2 的编号——本正典对这几条的表述见 §1.1.2（落盘即定型 / 字段集稳定 / 身份列）。**这些是「已落盘但不符合约束」的存量**，重构时一并改。` |
| §4 **列头** | `| 欠账 | 违反标准哪条 | 表现 |` | `| 欠账 | 违反哪条硬约束 | 表现 |` |
| §4 第 3 行「哪条」格 | `§4.1 路径规范` | `hive 分区命名（§1.1.2 落地形态）` |
...
| §5 表格最后一格 | `#1 #2 #3 #4 #5 #6 → 已进标准` | `#1 #2 #3 #4 #5 #6（**公司标准 §6 的编号；该标准尚未合入**）——本正典的案例库见 §1.6` |

> ⚠️ **三条踩过/被评审抓出的坑，照做**：
> ① **`§4.1 路径规范` 那格不能改指 §1.2** —— §1.2 里**没有** C1/C2/C3，也没有「路径规范」条目（它装的是 B7/B9/dbt 工件七项/数据面投递 + 流程约定）。C1/C2/C3 是**公司标准的编号**，全仓**无定义**，所以正确做法是**在 §4 里就地说明它们的出处**（即上面那行引用块），不是把读者指向一个同样没有它们的节。
> ② **§5 的 `#1…#6` 不是本正典 §1.6 的编号** —— 那是公司标准 §6 的案例号（标准 11 条 vs 本正典 17 条，**两套号码不同集**；标准 #6「主体只在路径」在本正典 §1.6 里甚至没有对应条）。⇒ **只标出处、不做映射**，别硬指 §1.6 的某几条。
> ③ **守卫要能覆盖这一格**：初稿给的 `grep -c "标准 §"` 对 `§4.1 路径规范` **结构性失明**（那格把「标准」二字省了）⇒ 0 计数**证明不了**「§4/§5 不再把未合入的标准当成现行」。改用**逐条列出**的判据（见 Step 4）。
...
**Expected:** `待补残留` 计数 `0`；`标准` 的每一处命中**都带「尚未合入」的括注**（逐条人工判，无一条把标准当现行）；§2/§3/§4 标题各打印一行；最后打印 `✓ 有`。
