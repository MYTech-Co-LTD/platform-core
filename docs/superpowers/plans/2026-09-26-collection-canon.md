...
> **为什么这条要写死**：2026-09-26 实测发现——零售采集每天绿、湖每天长，而 PG 里的物化**停在 09-23**
> （湖 09-24/25/26 都有，PG 没有；PG 那 19,678 行恰等于湖 09-23 一天；37 个 job 里**没有一个在跑 dbt**）
> ⇒ 报表读到的是 3 天前的数据，**且没有任何东西会报错**。这是「采集成功」最坏的一类假绿。
