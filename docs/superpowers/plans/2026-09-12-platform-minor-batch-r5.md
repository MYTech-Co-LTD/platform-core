...
**背景（用户已裁决）**：`server` 的宿主端口映射现在是 `'${HOST_PORT:-13000}:13000'`——**绑在 `0.0.0.0`**，即任何能访问宿主 13000 的人可绕过 edge 直连应用（丢掉了 edge 上的证书、限速、访问控制）。生产上它只经 openship edge 访问（edge 在宿主上反代 `127.0.0.1:<port>`，本仓实测过），故**绑回环是安全的**。**订正（T3 评审实测证伪原文）**：原文此处曾写「DB 端口此前已按同样理由改回环（R1 的 B4）」——**那是错的**，`postgres` 至今仍是 `5432:5432`（`0.0.0.0`），历史上从未改过（`git log --all -S'127.0.0.1:5432:5432'` 空结果）。评审并给出网络级 A/B（`nc` 打宿主 LAN IP：`-p 13502:5432` OPEN vs `-p 127.0.0.1:13503:5432` CLOSED），且证明「本地便利」站不住（文件用法、冒烟清单、dev-stack 全走 `127.0.0.1`）⇒ **协调者裁定一并绑回环**（见修复轮必须改 3）。
...
```yaml
      ...
      # （丢掉证书、限速与访问控制）。DB 端口同理（见修复轮必须改 3；原稿误称 R1 已改，实为未改）。
