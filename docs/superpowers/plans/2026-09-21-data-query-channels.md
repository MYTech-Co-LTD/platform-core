...
export async function touchPatKey(pool: Pool, org: string, id: number): Promise<void>   // ⚠️ 带 org：别写回 (pool, id)，见实现处的注记
 ...
 *  ⚠️ **`org` 是必需的**（2026-09-21 由 T3 评审报出、人裁决后补）：本函数原稿是 `(pool, id)`——
 *  那会是模块里**唯一**的裸 id 写路径，与全局约束 13「读写一律 `where org = $1`」相抵。
 *  靠「调用方只传 resolvePat 给的 keyId」是**约定**，不是机制；补上 org 才是机制。
 *  ⚠️ 与它相对：`resolvePat` **仍然不带 org**（`token_hash` 全局唯一，端口没有 org 位，补上成死结）——
 *  别把这一处的改法顺手推广过去。 */
...
- Modify: `modules/data/manifest.yaml`（`api.internal` 首批 **6 条**——原稿写 5 是漏数：GET /metrics、GET /metrics/all、POST /metrics、PUT /metrics/:id、DELETE /metrics/:id、POST /query。双向核对下 5 条不可交付，评审已判定 6 正确）
- Modify: `modules/data/index.ts`（注册 + 把 `execute` 透进 `RouteCtx` + **声明 `createPorts`**（约束 14）：把 T3 的 `resolvePat` 包成 `resolvePatKey(token)` 暴露给宿主。**这是宿主能解析 PAT 的唯一途径**——不声明的话 `runtime.port('data','resolvePatKey')` 恒 `undefined` ⇒ 通道 B 全线 503。
  ...
  **另：`requesterOf` 在此收 M3 的守卫**（T4 评审裁决）：`c.get('identity')` 缺失或 `identity.orgId` 为空串 ⇒
  返回 `null`（入口拒，走 runQuery 的 `unauthenticated`/fail-closed 路径），**别让空 org 流进 SQL**
  （下游虽是查空集 fail-safe，但「宁可入口拒」——一条 `if` 的事，别留给下游兜）。
...
| 38 | **T4 的测试清单自带两个缺口**（**由 T4 的任务评审报出，标 plan-mandated**——brief 钉死的用例清单，不是实施者漏写）：① **unauthenticated 结局的审计写入无断言**——匿名用例只查回包形状，「匿名被拒也留痕」（约束 7 的应有之义）没被验到，`query-service.ts` 的 audit 回归测不出；② **`warehouse_unconfigured` 映射零测试**——没有任何注入 `DATA_WAREHOUSE_UNCONFIGURED` 抛错形态的用例，`unconfigured` 与 `warehouse_error` 的分岔只有实现没有验证。两条都是 test-only，实现本身评审已确认正确 | T4 返工**只补测试不改实现**：① 补断言匿名调用后审计最新一行的 verdict/reason/user_id/channel/key_id；② 补一条按 T3 `warehouse.ts` **真抛错形状**注入的用例（mock 收严到真机形状——AGENTS 硬约束 11），断言回包 reason 是 `warehouse_unconfigured` 且审计行同步落 error。**M3 裁决（同轮）**：空 `org` 判 **Minor**——下游是查空集（fail-safe，不破隔离），守卫放 **T6 的 identity 装配层**（入口拒）优于在 `runQuery` 改钉死接口；可后续加 DB 层 CHECK 兜底 |
| 39 | **T6 的 manifest 首批条数写错（5 → 实为 6）**（T6 实施者报出、评审判定）：Files 行写「`api.internal` 首批 5 条」，但 brief 自身 Step 4 块与 Produces 均列 **6** 个端点（GET /metrics、GET /metrics/all、POST /metrics、PUT /metrics/:id、DELETE /metrics/:id、POST /query）——在装载期双向核对（约束 ⑪）下，5 条与 6 条注册**不可交付**，实施者按 6 落地是唯一自洽解。**教训**：写「N 条」这类计数时与端点清单**同段自检**——计数错了，双向核对会替你「纠正」成对的那个数，但照抄清单的人会先困惑 | Files 行改为 **6 条**并列全 6 个端点，注明原稿 5 是漏数、评审判定 6 正确 |
