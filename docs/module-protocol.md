...
403 会泄露"模块存在但被停用"，停用状态本身就成了可枚举信息）。闸门按**租户**在请求期判定，
...
- 闸门用 `app.use(base, gate)` + `app.use(base + '/*', gate)` 挂在**模块路由之前**。
...
- 无租户上下文时闸门放行给后续层（真实链路上租户中间件先于一切业务路由，闸门见到的请求必带
  租户；未命中 Host 的那类请求早已被租户中间件 404/抛错拦掉）。
