...
### CI 门禁（2026-09-16 立，issue #77）

spec-1 §2 原写「CI lint 待沉淀——第一个真模块落地后再评估」。**第一个真模块（售后，PR #74）
已于 2026-09-16 落地**，评估后立此门禁。判据 = **真库对账**，不是扫迁移文本：

- 脚本 `scripts/check-tenant-isolation.mjs`；**在 CI 的烟测 job 里紧跟 `smoke-load` 之后跑**
  ——那时宿主刚把全部模块 migrations 应用完，`information_schema` 就是**唯一真值**，
  不需要写 DDL 解析器，也不会被幂等写法（`if not exists`）与增量迁移骗到。
- **判据：模块 schema 下的每张表，其【累积终态】必须有 `org` 列。**
  ⚠️ **不按单个迁移文件判**——示例：`modules/demo/001_note.sql` 建表时**没有** org，
  是 `003_note_org.sql` 后补的。按文件判会把**本仓自己的参考模块**判红，
  而 demo 正是每个新模块照抄的模板 ⇒ 门禁一上来就会被关掉。
- **全局表的豁免**：在**建表语句处**写一行注释，**格式固定、必须带表名**

  ```sql
  -- global-table: aftersales.dict — 跨租户共享的字典表，无租户语义
  ```

  表名是**必须**的：脚本靠这一行拿到豁免清单，**不去解析 DDL**（解析建表语句是这类门禁
  最容易出错的部分）。脚本**只认这个标记**；上面那句「在模块 README 声明理由」是给
  **评审**看的散文，**机器读不到**，不作为豁免依据。理由与 DDL 同址，评审时才不会走散。
