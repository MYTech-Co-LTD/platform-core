...
## 跨模块只读数据依赖：`dataDependencies`（2026-09-23 拍板）

> 适用：`modules/<id>/manifest.yaml` 的**可选** `dataDependencies` 字段——消费方声明自己
> **只读**哪些**别的模块拥有的稳定视图**。规则以本节为准；架构背景（数据所有权、一源两面、
> 新鲜度分档、不做清单）读 `docs/architecture.md` §2.3；接入视角读 `docs/module-onboarding.md` §5。
>
> ⚠️ **本节是文档先行的契约**：manifest schema（`packages/platform-sdk/src/manifest.ts`）尚未收
> 该字段，B1 守卫也未扩——落地随采集首管线（架构 §2.3 实施注记的「四件套」）。落地前在
> manifest 里写它会被 `ManifestSchema.parse` 拒绝，这不是本节自相矛盾，是「先改文档、再写码」
> 的既定顺序。

**它解决的是什么**：本仓**第一个跨模块数据依赖**（`modules/aftersales` 消费数据栈操作面产出的
档案/订单数据）出现之前，先立规矩——不立，跨模块数据流会以「模块 A 直接 join 模块 B 的表」的
**隐式耦合**长出来；B1 能把这种引用拦个正着（gates 红），却说不出**该**怎么合法地做。
`dataDependencies` 就是那个「合法做法」的声明面：声明了、且只读稳定视图 = 合规；其余 = 违规。

### 消费方：怎么声明（**不声明 = 违规**，但拦截强度见下一节）

```yaml
# modules/<id>/manifest.yaml
dataDependencies:
  - data.v_order_snapshot        # 形状：<提供方 schema>.<稳定视图名>
```

- 条目必须是**稳定视图**（提供方对外承诺的只读面，`data.v_order_snapshot` 一类），**不是物理表**——
  防腐层语义：提供方内部 DDL 演进不破坏消费者（架构 §2.3 拍板③）。声明物理表属于违规。
- **只读**：`dataDependencies` 授权的是 `select`。对被依赖 schema 的任何写（insert / update /
  delete / DDL）都**不在授权面内**，写了即 B1 违规——**写侧无豁免**，引用表只有提供方模块
  （及它的 sync job）自己写。
- **声明 = 可读授权**，与 `api.internal` 的「声明即授权」同哲学：manifest 是模块能力的**唯一审计面**
  ——看 manifest 就知道谁读谁的数据。**不声明的跨 schema 读属违规**，无例外。

### 与 `api.internal` 的关键差异：这是文档契约 + 门禁层，**不是运行时门卫**

| | `api.internal` | `dataDependencies` |
|---|---|---|
| 未声明的后果 | **运行期 403**（fail-closed，宿主门卫逐请求强制） | **违规但运行期放行**——宿主不做 SQL 面的运行时拦截 |
| 守门人在哪 | 宿主包裹层门卫（`loader.applyDeclaredApiGate`） | **静态门禁 + 评审**：B1 读侧豁免以声明集为准；被依赖视图列集变更必须显式登记（见下节） |
| 为什么能/不能 | API 是宿主的路由面，宿主天然站在请求路径上 | 模块与宿主**共用同一个 `pool`**（`ModuleContext = { pool }`），跨 schema 读发生在模块自己的 SQL 文本里——宿主要拦等于解析每条 SQL，成本与误报都不可接受 |

⚠️ **别把「没拦」读成「允许」**：这条契约的执行强度在门禁与评审，不在运行时。这是它与
`api.internal` fail-closed 的**有意分叉**，不是缺口留白——模块代码里未声明的跨 schema 引用
会在 `gates` 的 B1 检查上报红（守卫扩面后按声明集判），只是不会在运行期 403。

### 门禁核什么（守卫实现留待实现期，**判据在这里定死**）

1. **声明与实现一致**：消费方代码里跨 schema 引用的视图集合 = manifest 声明集。
   `lint-architecture.mjs` 的 B1 **读侧豁免以声明为准**——`modules/<id>/` 里引用「声明过的
   `<提供方>.<视图>`」放行，引用**未声明的**任何跨 schema 对象（含物理表）仍报 B1。
2. **被依赖视图没被悄悄改**：提供方对**已被任何模块声明**的稳定视图做**列集变更**
   （加列 / 删列 / 改型 / 改名）时，门禁必须红——变更要先**显式登记**（同步更新消费方声明与
   迁移里的视图定义），否则就是静默漂移。核法候选：`check-manifests` 扩面或新守卫比对
   「声明集 ↔ 迁移里视图定义的列集」——**实现期定**，本节只锁判据。
3. **提供方物理表 DDL 演进自由**（这正是防腐层的意义），**前提是**稳定视图的对外形状不变；
   视图形状要变 = 走第 2 条的显式登记。
...
| 不变量 | 关系 |
|---|---|
| **B1 跨 schema（三同）** | **读侧开一个以声明为准的口子**（本节唯一动的一条）；**写侧无豁免**。守卫扩面是落地期的一步，判据见上节第 1 条 |
| **声明即授权（fail-closed）** | **同构但降一档强度**：声明=可读授权；未声明=违规。差异（运行时不拦）与理由见上节，勿混读成「同等 fail-closed」 |
| **装载期双向核对** | **不触犯**：它核对路由集合与 `api.internal` 的差集；本节不动路由。`dataDependencies` 的「双向核对」等价物是门禁第 1 条（声明 ↔ 代码引用一致），落在 gates 而非装载期 |
| **`enabledFor` 只能请求期门控** | **不触犯也不联动**：模块停用语义管的是「该租户看不看得到这个模块」，不管数据依赖；消费方停用提供方模块后的数据新鲜度属超周期场景——**已裁不做**（架构 §2.3 拍板④） |
| **B4/B5 manifest** | 落地时 schema 收字段（`manifest.ts` 的编译期双向断言会强制接口与 zod 同步）；`check-manifests` 是否加文件存在性检查按需定——视图在**真库**里，不在仓里，文件存在性检查对它无意义 |
