...
**代理会把流截断，而截断的响应是一个 `rc=0` 的 `HTTP 200`**——客户端**看不出任何异常**，
截断体的 `content-length` 是**自洽的**（它就是那么长）。⇒ 按「HTTP 200 即成功」写，装上去的是
**半截脚本**，而半截的 shell **未必语法报错**（可能只是缺了几段断言）——
**逐文件 sha256 断言是唯一能拦住它的东西**。

**截断不是固定窗口，而是随体积恶化**（2026-09-26 数据面机实测；同一个代理、同一个 curl，只换目标文件）：

| 目标 | 拿到 |
|---|---|
| 同仓库 13.6 KB 文件 | 完整 ✓ |
| 同仓库 79 KB 文件 | **34,354 B（43%）**，`rc=0` |
| 同仓库 190 KB 文件 | **57,258 B（30%）**，`rc=0` |
| **1.44 MB 管线**（本面**首次**遇到 MB 级工件） | **0 B**——`MAX_TRIES=10 × CURL_TIMEOUT=30` **10 次全败** |

⇒ 早期记的「截断在 9.3–11.3 KB 附近」只是当时那两次样本的落点，**不是上限、也不是规律**，
别把它当阈值用。

**⚠️ 根因尚未定案——别照抄任何单一解释。** 已实测**推翻**四条假设：① 代理本身对大 body 有问题
（**国内 4.17 MB 走代理：完整**）；② 境外那一跳有问题（**境外 npmjs 的 4.17 MB：也完整**）；
③ 缓存中毒（`cacheKey` 含 query ⇒ 两个**不同**键却拿到**同一份**截断体 ⇒ 那是可复现的截断）；
④ 路由判错（`githubusercontent.com`/`jsdelivr.net`/`npmjs.org` **同表同路**）。剩下的唯一事实是
「**同一代理、同一客户端、同一路由下：npmjs 的 4.17 MB 完整，而 raw.githubusercontent / jsdelivr
的 1.44 MB 截断**」⇒ **需在控制面读代理日志定位是哪一跳**（probe 回来的是 `206` 还是 `200`；
`[cache] PUT <key> (N bytes)` 里的 N 是否**已经是短数**）。跟踪：**`MYTech-Co-LTD/openship-platform#15`**。

**MB 级工件的现行口径（2026-09-26 起）**：投递面里一旦出现 MB 级文件，**先单独试取 3~5 次看成功率**，
再决定是否调 `MAX_TRIES`/`CURL_TIMEOUT`。**不要为了绕过它去改投递通道**（换 CDN 之类）——
那只是把根因藏起来，而这类**静默截断正是 sha256 断言存在的理由**。
