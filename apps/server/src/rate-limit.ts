 ...
 * `wechat-oa` = 公众号访客回调（售后 M1：外部匿名流量灌不满同租户另两扇门）。
 *
 ...
 *
 * 注意分的是**桶的键**，不是实例：各门仍共用 `app.ts` 里的同一个 limiter 实例，别实现成
 * 多个实例。
 *
 * 但**拆桶不是"不劈预算"**（PR#5 终轮评审 S1 更正：初稿这么写，与实测相反）。如实说：第 2/3
 * 层的额度是**每 (租户, 门) 一份**，故拆桶的代价是**聚合预算 ×门数**——每租户每分钟可写 audit
 * 的失败行数由 300 变 **900**（第 3 层 1000 变 **3000**），收益是**爆炸半径 ÷门数**（灌企微
 * 回调不再锁死账密路）。这是**有意的取舍**：只按租户合成一个跨门总闸，等于把 R1 那个"打一扇
 * 门即锁死两扇门"的缺陷原样复现，故不做。而第 1 层（单账号失败）**未**拆门，故"拆桶"严格
 * 优于"拆实例"（后者连第 1 层也一起放大，换不来任何额外收益）。
 *
 * 第 1 层（单账号失败）**不**分门：企微路本就没有用户名维度（spec §3.2），公众号访客路同样
 * 没有（openid 在 code 换票前拿不到），账密路的 user 桶保持 `(tenantId, username)` 不变。
 * 企微路/访客路的 record 一律传 username=null，天然不碰 user 桶。
 */
export type Door = 'password' | 'wecom' | 'wechat-oa'
